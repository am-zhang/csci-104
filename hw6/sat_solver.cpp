...
bool solveHelper(int currClause, int numClauses, map<int, int>& values, vector<vector<int>>& clauses); // recursive helper function

void solve(int numVars, int numClauses, map<int, int>& values, vector<vector<int>>& clauses); // calls solveHelper

bool isValid(map<int, int>& values, vector<int>& curr); // returns true if solution is found for one clause

bool verify(int numClauses, map<int, int>& values, vector<vector<int>>& clauses); // returns true if solution works for all clauses

bool solveHelper(int currClause, int numClauses, map<int, int>& values, vector<vector<int>>& clauses)
{
    ...
    for(unsigned int i=0; i<clauses[currClause].size(); i++) // iterates through every variable in the current clause
    {
        ...
        map<int,int>::iterator it = values.find(currVar);
        if(it != values.end())
        {
...
void solve(int numVars, int numClauses, map<int, int>& values, vector<vector<int>>& clauses)
{
    solveHelper(0, numClauses, values, clauses);
}

bool isValid(map<int, int>& values, vector<int>& curr)
{
...
bool verify(int numClauses, map<int, int>& values, vector<vector<int>>& clauses)
{
...
int main(int argc, char* argv[])
{
    ...
    map<int, int> values;
    for(int i=1; i<=numVars; i++) // populates map with unassigned values
    {
    ...
    else
    {
        ...
        {
            map<int,int>::iterator it = values.find(i);
            if(it != values.end())
            {
