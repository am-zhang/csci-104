...
bool TwitEng::parse(char* filename)
{
    ...
    while(!ifile.fail())
    {
        if(line != "") // skips blank lines
        {
            ...
            ss.ignore(); // ingores the space after the username
            string tweet_text;
...
void TwitEng::addTweet(const string& username, const DateTime& time, const string& text)
{
    ...
    if (new_it != all_users.end())
    {
        ...
        vector<string> store_mention = new_tweet->mentions();
        if(new_tweet->isMention) // if there's at least one mention in the tweet
        {
            if(new_tweet->startMention) // if starts with mention, adds mention to mention feed of user
            {
                std::map<string, User*>::iterator it_mention = all_users.find(store_mention[0]);
                if (it_mention != all_users.end())
                {
                    it_mention->second->addMention(new_tweet);
                }
            }
            else // other mentions show up in mention feeds of mentioned users
            {
                for(unsigned int i=0; i<store_mention.size(); i++)
                {
                    std::map<string, User*>::iterator it_mention = all_users.find(store_mention[i]);
                    if (it_mention != all_users.end())
                    {
                        it_mention->second->addMention(new_tweet);
                    }
...
void TwitEng::dumpFeeds()
{
    ...
    {
        string file_name = it->first + ".feed"; // names a new file
        ...
        string mention_file = it->first + ".mentions";
        ofstream mfile(mention_file);
        mfile << it->first << endl;
        vector<Tweet*> mention_tweet = (it->second)->getMentionsFeed();
        for (unsigned int i=0; i<mention_tweet.size(); i++) // getFeed() returns vector of Tweet*
        {
            mfile << *(mention_tweet[i]); // dereferences tweet and outputs using overloaded <<
            if(i < mention_tweet.size()-1)
            {
                mfile << endl; // takes care of newlines
            }
        }
    }
}

void TwitEng::addFollower(string user, string follow)
{
    std::map<string, User*>::iterator it_user = all_users.find(user);
    if (it_user == all_users.end()) // checks if user exists
    {
       cout << user << " is not a current user" << endl;
       return;
    }

    std::map<string, User*>::iterator it_follow = all_users.find(follow);
    if (it_follow == all_users.end()) // checks if user exists
    {
        cout << follow << " is not a current user" << endl;
        return;
    }

    it_user->second->addFollowing(it_follow->second);
    it_follow->second->addFollower(it_user->second);
}

void TwitEng::save(string filename)
{
    ofstream ofile(filename);
    ofile << all_users.size() << endl; // outputs number of users
    std::map<string, User*>::iterator it;
    for(it = all_users.begin(); it != all_users.end(); ++it) // outputs all users and their followers
    {
        ofile << it->first << " "; // outputs the user
        set<User*> user_following = it->second->following(); // outputs their followers
        for(set<User*>::iterator it_follow = user_following.begin(); it_follow != user_following.end(); ++it_follow)
        {
            ofile << (*it_follow)->name() << " ";
        }
        ofile << endl;
    }
    for (unsigned int i=0; i<all_tweets.size(); i++)
    {
        ofile << *(all_tweets[i]) << endl;
    }
    ofile.close();
}